...
trace_specs = [
    ...
    # ----- web -----
    ('web.application.application.__init__', HL),

    ('web.application.application._cleanup', HL),

    ('web.application.application._delegate', HL),

    ('web.application.application._load', HL),

    ('web.application.application._match', HL),

    ('web.application.application._unload', HL),

    ('web.application.application.add_processor', HL),

    ('web.application.application.init_mapping', HL),

    ('web.application.application.load', HL),

    ('web.application.application.handle', HL),

    ('web.application.application.handle_with_processors', HL),

    ('web.application.application.wsgifunc', HL),

    ('web.application.loadhook', HL),

    ('web.application.Reloader.check', 'hide_tree'),

    ('web.application.unloadhook', HL),

    ('web.utils.ThreadedDict.__setattr__', False),
...
def printing_handler_filter_func(info):
    ...
    # If is this URI
    if onwrap_uri == 'web.application.application.load':
        # Get arguments inspect info
        arguments_inspect_info = info['arguments_inspect_info']

        # Get `env` argument info
        arg_info = arguments_inspect_info.fixed_arg_infos['env']
...
def main():
    try:
        ...
        url_to_handler_mapping = ('.*', CustomRequestHandler)

        # Get application
        webpy_app = web.application(
            mapping=url_to_handler_mapping,
            autoreload=False,
        )

        # Get WSGI application
        wsgi_app = webpy_app.wsgifunc()
